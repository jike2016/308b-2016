...
/** Start 获取组织机构的全部数据  朱子武 20160314*/
$orgData = $DB->get_records_sql('select id, name from mdl_org');
$org_name = array();
foreach($orgData as $org_value)
{
    $org_name[] = $org_value->name;
}
/** End 获取组织机构的全部数据  朱子武 20160314*/

// array of all valid fields for validation
$STD_FIELDS = array('id', 'username', 'email','org', // wepeng 20160313
...
//            }
        ...
        /**wepeng 20160313 如果存在组织架构则更新组织架构 朱子武 20160314 */

        if (isset($user->org)) {

            $org_obj = new stdClass();

//            $org_value_id = '';
            $org_obj->org_id = '';
            foreach($orgData as $org_value_obj)
            {
                if($org_value_obj->name === $user->org)
                {
                    $org_obj->org_id = $org_value_obj->id;
                    $org_obj->user_id = $user->id;
                    $DB->insert_record('org_link_user',$org_obj);
                }
            }
        }
        /** END */
...
while ($linenum <= $previewrows and $fields = $cir->next()) {
    ...
    /**  Start 判断组织机构名称是否合法 朱子武 20160314*/
    if(isset($rowcols['org']))
    {
        if(!in_array(str_replace(' ', '', $rowcols['org']), $org_name))
            $rowcols['status'][] = '组织机构名称有误，请检查';
    }
    /**  End 判断组织机构名称是否合法 朱子武 20160314*/
    ...
    $rowcols['status'] = implode('<br />', $rowcols['status']);
	 ...
	 /** Start 预览错误，提示用户 朱子武 20160316*/
	 // $data[] = $rowcols;
    if(!($rowcols['status'] =='')) $data[] = $rowcols;
    /** End 预览错误，提示用户 朱子武 20160316*/
}
if ($fields = $cir->next()) {

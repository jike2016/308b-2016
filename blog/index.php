...
/** Start 岑霄 20160310*/
$mycontent  = optional_param('mycontent', 0, PARAM_INT);//0:全部1：我的关注2：与我相关
/** End */
comment::init();
...
//if(!has_capability('moodle/blog:manageentries', $context)){
//    $PAGE->set_pagelayout('incourseforstudent');
//    //默认为incourse管理员可见
//}
// $PAGE->set_pagelayout('missionmy');


/** START 20160309 重新为学习圈设置Layout */
$PAGE->set_pagelayout('blogmy');
/** End  */
...
/** START 徐东威20160309 注释掉，解决layout冲突 */
...
/** START 徐东威20160309 调整参数输出render */
// $bloglisting = new blog_listing($blogheaders['filters']);


if($entryid != null){
    $bloglisting = new blog_listing(array('entry'=>$entryid));
}
elseif($userid != null){
//    $bloglisting = new blog_listing();//替换上面的语句
    $bloglisting = new blog_listing(array('user'=>$userid));
}
else{
    $bloglisting = new blog_listing();//替换上面的语句
}
/** End*/
/** SATRT 判断类型 朱子武 20160310*/
if($mycontent==0){//全部
	$bloglisting->print_entries();
}
elseif($mycontent==1){//我的关注
    echo myconcerned();
}
elseif($mycontent==2){//与我相关
    echo myrelated();
}
/** End*/
echo $OUTPUT->footer();
...
function myconcerned()
{
    ...
    global $OUTPUT;
    global $CFG;
    $myconcernedResult = $DB->get_records_sql('SELECT a.*, b.firstname, b.lastname FROM mdl_blog_concern_me_my a JOIN mdl_user b ON a.concernid = b.id WHERE userid = '.$USER->id);
		...
		<script>
			$(document).ready(function() {
				$(".people-box").click(function() {
                    ...
                    window.open(window.location.protocol+"//"+window.location.host+"/moodle/blog/index.php?userid="+userid);
				});

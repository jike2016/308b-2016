...
var ajaxChat = {
	...
	usersList: null,
	userNamesList: null,
	usersList_new: null,  //20160331 wepeng 在线列表
	userNamesList_new: null, //20160331 wepeng 在线列表
	...
	init: function(config, lang, initSettings, initStyle, initialize, initializeFunction, finalizeFunction) {	
		...
		this.usersList			= [];
		this.userNamesList		= [];
		this.usersList_new			= []; //20160424 wepeng 在线列表
		this.userNamesList_new		= []; //20160424 wepeng 在线列表
	...
	handleXML: function(xmlDoc) {
		...
		this.handleOnlineUsers(xmlDoc.getElementsByTagName('user'));
		this.handleChatMessages(xmlDoc.getElementsByTagName('message'));
		//20160331 wepeng 添加获取在线用户
		this.handleOnlineUsers_new(xmlDoc.getElementsByTagName('user_online'));
		//end
	...
	handleOnlineUsers: function(userNodes) {  //wepeng(20160301) 修改在线列表为未读消息列表
		if(userNodes.length) {
			var index,userID,userName,userRole,i,channelName,
				onlineUsers = [];
			for(i=0; i<userNodes.length; i++) {
				...
				channelName = userNodes[i].lastChild ? userNodes[i].lastChild.nodeValue : '';
	...
	handleOnlineUsers_new: function(userNodes) {  //wepeng(20160331) 添加在线列表功能，还需要界面显示功能的修改
		if(userNodes.length) {
			...
			for(i=0; i<userNodes.length; i++) {
				...
				onlineUsers_new.push(userID);
				//alert('onlineUsers_new:'+userID);
				index = this.arraySearch(userID, this.usersList_new);
				//alert('index:'+index);
				//alert(userID+":"+userName+":"+index);
				if(index === false) {
					this.addUserToOnlineList_new(
						userID,
						userName,
						channelName
					);
				} else if(this.userNamesList_new[index] !== userName) {
					this.removeUserFromOnlineList_new(userID, index);
					this.addUserToOnlineList_new(
						userID,
						userName,
						channelName
			...
			// Clear the offline users from the online users list:
			for(i=0; i<this.usersList_new.length; i++) {
				if(!this.inArray(onlineUsers_new, this.usersList_new[i])) {
					this.removeUserFromOnlineList_new(this.usersList_new[i], i);
				}
			}	
			//this.setOnlineListRowClasses();		
		}	
	...
	removeUserFromOnlineList_new: function(userID, index) { //wepeng 20160424 修改
		//alert('removeUserFromOnlineList_new:'+userID);
		this.usersList_new.splice(index, 1);
		this.userNamesList_new.splice(index, 1);	
		//去掉在线图标
		var zTree = $.fn.zTree.getZTreeObj("tree");
		nodeList = zTree.getNodeByParam("userid", userID);
		nodeList.name = nodeList.name.replace('(在线)', '');
		zTree.updateNode(nodeList);
		//document.getElementById("tree_3_span_online").remove();
		//if(this.dom['onlineList']) {
		//	this.dom['onlineList'].removeChild(this.getUserNode(userID));
		//}
	},
		
	addUserToOnlineList: function(userID, userName, channelName) {  //wepeng 20160301 修改在线列表为未读消息列表
		this.usersList.push(userID);
		this.userNamesList.push(userName);	
		if(this.dom['onlineList']) {
			this.updateDOM(
				'onlineList',
				this.getUserNodeString(userID, userName, channelName),
				(this.userID === userID)
			);
		}
	},
	
	addUserToOnlineList_new: function(userID, userName, channelName) {  //wepeng 20160424 修改
		this.usersList_new.push(userID);
		this.userNamesList_new.push(userName);	
		//添加在线图标
		var zTree = $.fn.zTree.getZTreeObj("tree");
		nodeList = zTree.getNodeByParam("userid", userID);
		nodeList.name = userName + '(在线)';
		zTree.updateNode(nodeList);
	},

	getUserNodeString: function(userID, userName, channelName) { //wepeng 20160301 修改在线列表为未读消息列表
		...
		if(this.userNodeString && userID === this.userID) {
			...
			str	= '<div id="'
					...
					+ '"><a onclick="ajaxChat.switchChannel(\''
					+ channelName
					+ '\');document.getElementById(\'PublictButton\').value = \''
					+ userName
					+ '\';" class="'
					+ this.getRoleClass(1)
					+ '" title="'
	...
	getChatListMessageString: function(dateObject, userID, userName, userRole, messageID, messageText, channelID, ip) {
		...
		//20160419 wepeng 修改自己的消息为右对齐
		var dateTime_temp = this.settings['dateFormat'] ? this.formatDate(this.settings['dateFormat'], dateObject): '';
		if(userID == this.userID)
		{
			return	'<div id="'
				...
				+ dateTime_temp
				+ '</span>'
				+ '<span style="float:right" dir="ltl">'

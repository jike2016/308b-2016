...
//            标题
            ...
            if($my_originalblogid == '') {
                ...
                // 设置内容
                $my_result[$blogid]->summary = '[转发]'.$my_result[$blogid]->summary . '<br/><p>原文地址：</p><a href = ' . $blogurl . '>' . $blogtitle . '</a>';
                // 插入数据库
...
//            查询是否已经点赞
            ...
            {
                $my_result = $DB->get_records_sql('SELECT id, userid, subject FROM mdl_circles_of_learning WHERE id = '.$blogid);
...
//    搜索学员信息 朱子武 20160315
    ...
    case 'comment':
        ...
        $myresult = $DB->get_records_sql('SELECT id, userid, subject FROM mdl_circles_of_learning WHERE id = '.$newcmt->itemid);

        $blogtitle= $myresult[$newcmt->itemid]->subject;
        $blogurl = '/blog/index.php?entryid='.$newcmt->itemid;
        $newrelated =  new stdClass;

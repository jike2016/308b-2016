...
//  Display the course home page.
    ...
    if($DB->record_exists('role_assignments', array('roleid' => $role->get_gradingadmin_role(),'userid' => $USER->id))){
        $role_flag = 1;//分级管理员
        $return_indexURL = '/course/course_classify/index_gradingAdmin.php';
    }
    else if($DB->record_exists('role_assignments', array('roleid' => $role->get_courseadmin_role(),'userid' => $USER->id))){
        $role_flag = 2;//慕课管理员
        $return_indexURL = '/course/course_classify/index_courseAdmin.php';
    }
    ...
    $PAGE->set_url('/course/course_classify/view_my.php', $urlparams); // Defined here to avoid notices on errors etc
	...
	/** Start 页面layout设置 **/
    if($role_flag == 1){
        $PAGE->set_pagelayout('courseforgradingadmin');//设置layout文件,分级管理员
    }
    else if($role_flag == 2){
        $PAGE->set_pagelayout('courseforadmin');//设置layout文件，慕课管理员
    }
	/** End**/

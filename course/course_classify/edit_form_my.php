...
//        }
        ...
        if (empty($course->id)) {
			//添加
            //if (has_capability('moodle/course:create', $categorycontext)) {//角色权限判断，这里不用此判断
            if(true){
                //$displaylist = coursecat::make_categories_list('moodle/course:create');
                $displaylist = self::getCategoryList();
				...
				//加入多选
				$select = $mform->addElement('select', 'multicategory', get_string('coursecategory'), $displaylist);
				$select->setMultiple(true);
                $mform->addHelpButton('multicategory', 'coursecategory');
                $mform->setDefault('multicategory', $category->id);
                $mform->addRule('multicategory', '请选择一个类别', 'required', null, 'client');
				//加入结束
				//加入全局标签
				$tag_my=self::gettagmylist();
				$select = $mform->addElement('select', 'multitagmy', '标签选择', $tag_my);
				$select->setMultiple(true);
                $mform->addHelpButton('multitagmy', 'coursecategory');
                //选中默认
                foreach($tag_my as $key => $tag_my_default){
                    $tag_default=$key;
                    break;
                }
                $mform->setDefault('multitagmy', $tag_default);
                $mform->addRule('multitagmy', '请选择一个标签', 'required', null, 'client');
				//加入结束
            } else {
			...
			//修改
            //if (has_capability('moodle/course:changecategory', $coursecontext)) { //课程修改权限
            if(true){
                $displaylist = coursecat::make_categories_list('moodle/course:create');
                ...
                //获取默认选中项
				 global $DB;
				 $coursecategories = $DB->get_records('course_link_categories', array('mdl_course_id'=>$course->id));
                 $arr=array();
                 foreach($coursecategories as $value){
                     $arr[]=$value->mdl_course_categories_id;
                 }
				 $mform->getElement('multicategory')->setSelected($arr);
                 $mform->addRule('multicategory', '请选择一个类别', 'required', null, 'client');
				 //加入结束
                ...
                $mform->addHelpButton('multitagmy', 'coursecategory');
				//获取默认选中项
                $tag_selected = $DB->get_records('tag_link', array('link_name'=>'mdl_course' , 'link_id'=>$course->id));
				    $arr=array();
				    foreach($tag_selected as $value){
					    $arr[]=$value->tagid;
				    }
                $mform->getElement('multitagmy')->setSelected($arr);
                $mform->addRule('multitagmy', '请选择一个标签', 'required', null, 'client');
				//加入结束
...
//        }
        ...
        /** Start 推送考试提醒复选框 02.27 毛英东 */
        $mform->addElement('checkbox', 'pushnewcourse', "推送新课程提醒");
        /** End */
...
//        }
    ...
    {
        global $DB;
        $categorys = $DB->get_records_sql("select cc.id,cc.name from mdl_course_categories cc where cc.depth = 1");
        $category_array = array();
        foreach ($categorys as $category) {
            $category_array[$category->id] = $category->name;
            $sub_category = self::getSubCategory($category->id,$category->name);
    ...
    //递归获取子级分类
    function getSubCategory($categoryID,$categoryName){
        global $DB;
        $sbu_categorys = $DB->get_records_sql("select cc.id,cc.name from mdl_course_categories cc where cc.parent = $categoryID");
        $sbu_category_array = array();
        foreach($sbu_categorys as $sbu_category){
            $sbu_category_array[$sbu_category->id] = $categoryName.'/'.$sbu_category->name;
            $third_category = self::getSubCategory($sbu_category->id,$categoryName.'/'.$sbu_category->name);//递归调用
            foreach($third_category as $key=>$value){
                $sbu_category_array[$key] = $value;
            }
        }
        return $sbu_category_array;
    }
	...
	{
		global $DB;
		$tag_datas = $DB->get_records_sql('select * FROM mdl_tag_my;');
		$tag_my = array();
		foreach ($tag_datas as $tag_data) {
			$tag_my[$tag_data->id] = $tag_data->tagname;
		}
		return $tag_my;
	}
}

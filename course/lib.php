...
function create_course($data, $editoroptions = NULL) {
	...
	for ($x=0; $x<count($data->multicategory); $x++) {
		$DB->insert_record('course_link_categories',array('mdl_course_id'=>$newcourseid, 'mdl_course_categories_id'=>$data->multicategory[$x]) );
	} 
	//结束
	//课程多标签插入
    $tag_link=new stdClass();
	for ($x=0; $x<count($data->multitagmy); $x++) {
		$tag_link->tagid = $data->multitagmy[$x];
		$tag_link->link_name = 'mdl_course';
		$tag_link->link_id = $newcourseid;
		$DB->insert_record('tag_link',$tag_link,true);
		//更新tag_my表里的num
		update_tagmy_num();
	} 
	//结束

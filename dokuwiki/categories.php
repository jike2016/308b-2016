...
foreach ($first_categories as $cagetory){
	//找出当前的一级词条名称
	if($cagetory->id == $_GET['categoryid']){
		$current_category = $cagetory;
	}

}
//查询一级分类最新的20个词条
$first_words = $DB->get_records_sql('select id,word_name from mdl_dokuwiki_word_my where categoryid='.$_GET['categoryid'].' order by create_time desc LIMIT 0,'.$numperpage);
//查询各二级分类和点击量前20的词条
$second_categories = $DB->get_records_sql('select id,name from mdl_dokuwiki_categories_my where parent = '.$_GET['categoryid']);
$second_words = array();
foreach($second_categories as $category){
	$second_words[$category->id] = $DB->get_records_sql('select id,word_name from mdl_dokuwiki_word_my where categoryid='.$category->id.' order by create_time desc LIMIT 0,'.$numperpage);
}
...
<html lang="<?php echo $conf['lang'] ?>" dir="<?php echo $lang['direction'] ?>" class="no-js">
	...
	<div class="main">
			...
			<p class="more"><a href="category_single.php?categoryid=<?php echo $_GET['categoryid'];?>">更多>></a></p>
		<!--一级分类板块 end-->
			...
			<?php
				...
				foreach($second_categories as $second_category){
					$output_my .= '<div class="second_classification">
									<h2 class="title">'.$current_category->name.' :  <span class="second_title">'.$second_category->name.'</span></h2>
									<table class="table">
										<tbody>';
					foreach($second_words[$second_category->id] as $second_word){
						if($n % 5 -1  == 0 || $n == 1){
							$output_my .= '<tr>';
						}
						$output_my .= '<td><a href="http://'.$_SERVER['HTTP_HOST'].'/moodle/dokuwiki/doku.php?id='.$second_word->word_name.'">'.$second_word->word_name.'</a></td>';
						if($n % 5 == 0){
							$output_my .= '</tr>';
						}
						$n++ ;
					...
					$output_my .= '		</tbody>
								...
								<p class="more"><a href="category_single.php?categoryid='.$second_category->id.'">更多>></a></p>';
				}
				echo $output_my;
			?>
		<!--二级分类板块 end-->

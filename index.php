...
if (!empty($CFG->customfrontpageinclude)) {
    ...
    if (($section && (!empty($modinfo->sections[1]) or !empty($section->summary))) or $editing) {
        ...
        if ($editing && has_capability('moodle/course:update', $context)) {
            $streditsummary = get_string('editsum	mary');
			/** Start 20160112 注释，去掉主页的编辑按钮 岑霄**/
...
/** Start 20160112 修改主页内容输出 岑霄**/
$indexpagerenderer = $PAGE->get_renderer('core', 'course');
$indexpagerenderer->my_print_indexpage();
/** End **/

foreach (explode(',', $frontpagelayout) as $v) {//选择首页输出内容
    switch ($v) {
        ...
        case FRONTPAGENEWS:
            if ($SITE->newsitems) {
                ...
                forum_print_latest_discussions($SITE, $newsforum, $SITE->newsitems, 'plain', 'p.modified DESC');//输出微阅新闻
        ...
        case FRONTPAGEENROLLEDCOURSELIST:
            $mycourseshtml = $courserenderer->frontpage_my_courses();//render输出课程

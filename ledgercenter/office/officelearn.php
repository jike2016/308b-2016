...
//	');
	...
	$totalPeople = get_totalPeople_org($org);//子单位总人数
	//积极榜处理
...
//	$rank_active[]= array('orgname'=>$org->name,'avgcount'=>$avg_org_active,'totalPeople'=>count($sumusers));
	$totalLearnEvent = get_totalActiveEvent_org($org,$start_time,$end_time);//子单位下的总事件完成数
...
//	$rank_learn[]= array('orgname'=>$org->name,'avgcount'=>$avg_org_learn,'totalPeople'=>count($sumusers));
	$totalLearnEvent = get_totalLearnEvent_org($org,$start_time,$end_time);//子单位下的总事件完成数
 ...
 */
function get_totalLearnEvent_org($org,$start_time,$end_time){
	...
	foreach($subOrgs as $subOrg){
		$subOrgEvent = $subOrgEvent + get_totalLearnEvent_org($subOrg,$start_time,$end_time);//这里递归调用
	}
 ...
 * @ return  totalEvent 当前子单位下的‘总事件数’;
 */
function get_totalActiveEvent_org($org,$start_time,$end_time){
	...
	$subOrgEvent = 0;//各下级子单位的人员完成事件之和
	foreach($subOrgs as $subOrg){
		$subOrgEvent = $subOrgEvent + get_totalActiveEvent_org($subOrg,$start_time,$end_time);//这里递归调用
	}
	$totalEvent = $totalEvent + $subOrgEvent;//当前单位下的人员完成事件 + 当前单位的下级子单位人员完成事件

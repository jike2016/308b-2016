...
foreach($sumorgs as $org){
    ...
    $totalPeople = get_totalPeople_org($org);//获取子单位的人员数

    //单位浏览数
//    $orgBrowsNum = browseNum($org->id,$start_time,$end_time) / (count($sumusers));
//    $rank_browseNum["$org->name"] = $orgBrowsNum;
    $totalBrows = browseNum_org($org,$start_time,$end_time);//总浏览数
...
//    $rank_upload["$org->name"] = $orgUpload;
    $totalUpload = upload_org($org,$start_time,$end_time);//总上传数
...
//    $rank_passCheck["$org->name"] = $orgBrowsNum;
    $totalPassCheck = upload_org($org,$start_time,$end_time);//总通过审查数
 ...
 */
function get_totalPeople_org($org){
    ...
    foreach($subOrgs as $subOrg){
        $subOrgPeople = $subOrgPeople + get_totalPeople_org($subOrg);//这里递归调用
    }
...
/**Start 获取单位浏览数 xdw */
function browseNum_org($org,$start_time,$end_time){
    ...
    $subOrgs = $DB -> get_records_sql('select id,name from mdl_org where parent='.$org->id);
    foreach($subOrgs as $subOrg){
        $count_subOrg = $count_subOrg + browseNum_org($subOrg,$start_time,$end_time);//递归调用
    }
...
/**Start 获取单位上传数 xdw */
function upload_org($org,$start_time,$end_time){
    ...
    $subOrgs = $DB -> get_records_sql('select id,name from mdl_org where parent='.$org->id);
    foreach($subOrgs as $subOrg){
        $count_subOrg = $count_subOrg + upload_org($subOrg,$start_time,$end_time);//递归调用
    }
...
/**Start 获取单位通过审查数 xdw */
function passCheck_org($org,$start_time,$end_time){
    ...
    $subOrgs = $DB -> get_records_sql('select id,name from mdl_org where parent='.$org->id);
    foreach($subOrgs as $subOrg){
        $count_subOrg = $count_subOrg + passCheck_org($subOrg,$start_time,$end_time);//递归调用
    }

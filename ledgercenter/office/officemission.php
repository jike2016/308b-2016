...
foreach($sumorgs as $org){
    ...
    $orgMission = handle_mission_org($org,$mission);
 ...
 * @return object $missionObj  包含：单位名称 接受任务人数 平均完成必修课 平均完成选修课
 */
function handle_mission_org($org,$mission){
    ...
    //获取该单位下该任务的人员
    $missionUsers = get_missionPeople_org($org,$mission); //获取该单位下》该任务》相关人员
 ...
 */
function get_missionPeople_org($org,$mission){
    ...
    foreach($subOrgs as $subOrg){
        $missionPeople_SubOrg_temp = get_missionPeople_org($subOrg,$mission);//递归调用

...
class coursecat implements renderable, cacheable_object, IteratorAggregate {
    ...
    protected static function get_course_records($whereclause, $params, $options, $checkvisibility = false) {
		...
		//多属性，加入对表course_link_categories的查询
		$sql = "SELECT ". join(',', $fields). ", $ctxselect
                FROM {course} c
                JOIN {context} ctx ON c.id = ctx.instanceid AND ctx.contextlevel = :contextcourse JOIN mdl_course_link_categories d ON d.mdl_course_id = c.id
                WHERE ". $whereclause." ORDER BY c.sortorder";
    ...
    public function get_courses($options = array()) {
        ...
        if ($recursive) {
			...
			//查表mdl_course_link_categories
			$where .= 'AND d.mdl_course_categories_id = :categoryid';
	...
	public static function my_calculate_num_of_course($courses){
		if(count($courses)==0){
			return $courses;
		}
		//判断是否已查询过学生数量
	...
	//获取summary中的图片
	public static function my_get_course_formatted_summary_pix($course) {
        global $CFG;
		...
		$my_course_summary_pix=coursecat::my_search_summary_pix($course->summary);
	...
	public static function my_search_summary_pix($summary){
		$pix_start='src="';
		$pix_end='"';	

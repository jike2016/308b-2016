...
function delete_course($courseorid, $showfeedback = true) {
	...
	//加入删除表删除课程也要删除表course_link_categories对应记录
	$DB->delete_records("course_link_categories", array("mdl_course_id" => $courseid));
	//结束
	
	//删除课程也要删除表tag_link对应记录
	$DB->delete_records("tag_link", array("link_name" => 'mdl_course', "link_id" => $courseid));

    //删除课程也要删除表mdl_course_order_my对应记录
	$DB->delete_records("course_order_my", array("courseid" => $courseid));

    //删除课程也要删除表mdl_score_course_sum_my对应记录
    $DB->delete_records("score_course_sum_my", array("courseid" => $courseid));
	//更新tag_my表里的num
	update_tagmy_num();
	//结束
...
function remove_course_contents($courseid, $showfeedback = true, array $options = null) {
    ...
    foreach ($locations as $modname => $moddir) {
		...
		if ($modname === 'teamlearn') {
            continue;
        }
        if ($modname === 'missionmy') {
            continue;
        }
        if ($modname === 'notemy') {
            continue;
...
function message_popup_window() {
    ...
    if ($validmessages > 0) {
        ...
        /** Start 毛英东 消息提醒左移  'style' => "right:60px" 20160228 */
        $content =  html_writer::start_tag('div', array('id' => 'newmessageoverlay', 'class' => 'mdl-align', 'style' => "right:60px;")).
        //$content =  html_writer::start_tag('div', array('id' => 'newmessageoverlay', 'class' => 'mdl-align')).
            /** End */
                        html_writer::start_tag('div', array('id' => 'newmessagetext','style' => "padding-left:10px")).'<span class="glyphicon glyphicon-bell"></span>'.
                            $strmessages.
                        ...
                        html_writer::end_tag('div');
                    html_writer::end_tag('div');
		$content2 ='<div class="news">
			<div>
				<p><span class="glyphicon glyphicon-bell" ></span>新消息（</p><p class="num">1</p><p>）</p>
			</div>
			<a href="#">看消息</a>

...
$ebooks = $DB->get_records_sql('select 
	a.id,a.name,a.author,a.summary,a.url,a.pictrueurl,a.timecreated,a.wordcount,a.suffix,a.size,b.name as categoryname 
	FROM mdl_ebook_my a 
		...
		/**START cx 循环输出当前页电子书*/
			...
			foreach($ebooks as $ebook){
				...
				<tr target="ebookid" rel="'.$ebook->id.'">
					<td>'.$offset.'</td>
					<td>'.$ebook->name.'</td>
					<td>'.$ebook->pictrueurl.'</td>
					<td>'.$ebook->categoryname.'</td>
					<td>'.$ebook->author.'</td>
					<td>'.$ebook->summary.'</td>
					<td>'.$ebook->timecreated.'</td>
					<td>'.$ebook->suffix.'</td>

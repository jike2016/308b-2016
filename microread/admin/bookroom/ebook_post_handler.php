...
if(isset($_GET['title']) && $_GET['title']){
	...
	switch ($_GET['title']){
		...
		case "edit"://编辑
			...
			$newebook->author= $_POST['author'];
			$newebook->summary= $_POST['summary'];
			$newebook->url= $_POST['url'];
			$newebook->pictrueurl= $_POST['pictrueurl'];
			$newebook->timecreated= time();
	...
	if(isset($_FILES['pictrueurl'])){//上传图片
		if ($_FILES["pictrueurl"]["error"] > 0){
			...
			if ($_FILES["url"]["error"] > 0){//上传下载的文件
				...
				$newebook->author= $_POST['author'];
				$newebook->summary= $_POST['summary'];
				$newebook->url= $_FILES["url"]["name"];
				$newebook->pictrueurl= $_FILES["pictrueurl"]["name"];
				$newebook->timecreated= $currenttime;
				$newebook->wordcount= $_POST['wordcount'];
				$newebook->suffix= $_FILES["url"]["type"];
				$newebook->size= ($_FILES["url"]["size"] / 1024).'KB';
				global $DB;

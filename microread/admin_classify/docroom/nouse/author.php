...
if(isset($_POST['sumnum'])){
	...
	$sumnum = $DB->get_record_sql('select count(*) as sumnum from mdl_doc_author_my a '.$sql.'');
	$sumnum = $sumnum->sumnum;
}
//查询当前页记录
$offset = ($pagenummy-1)*$numPerPage;//获取limit的第一个参数的值 offset ，假如第一页则为(1-1)*10=0,第二页为(2-1)*10=10。
$authors=$DB->get_records_sql('select *from mdl_doc_author_my a '.$sql);//作者数量少基本不会有第二页。。。所以先不做分页了
?>
...
<div class="pageContent">
    <div class="panelBar">
        <ul class="toolBar">
            <li><a class="add" href="docroom/author_add.php" target="dialog"><span>添加</span></a></li>
            <li><a class="delete" href="docroom/author_post_handler.php?title=delete&authorid={authorid}" target="ajaxTodo" title="确定要删除吗?"><span>删除</span></a></li>
            <li><a class="edit" href="docroom/author_edit.php?authorid={authorid}" target="dialog"><span>修改</span></a></li>
        </ul>
    </div>

$(function(){
	...
	$('.img-list').infinitescroll(
		...
		function(newElems) {
			...
			$(newElems).filter(".img-item").click(function(){
				$(".onclickimg").removeClass("onclickimg");
				$(this).addClass("onclickimg");
				//$(this).children('a').children('img').attr('height');
				var path = $(this).children('a').children('img')[0].src;
				$(".fs_gallery_shuft_item img").attr('src',path);
				var winH = $(window).height();
				var imgbox = winH - 180;	
				$(".fs_gallery_shuft").css({"height":winH});
				$(".fs_gallery_shuft_item").css({"height":imgbox});
				$(".fs_gallery").show();				
			});

...
/** 播放器的自动跳转 */
$autocontinue = optional_param('autocontinue', 0, PARAM_INT);
$currenturl1 = optional_param('currenturl1', '', PARAM_RAW);
/** 播放器的自动跳转 */
/** 视频时间清零 */
if($autocontinue==1){
    if($DB->record_exists('videoseek_my', array('videourl'=>$currenturl1,'userid'=>$USER->id))){
        //更新数据
        $response=$DB->get_record('videoseek_my',array('videourl'=>$currenturl1,'userid'=>$USER->id));
        $DB->update_record('videoseek_my', array('id'=>$response->id,'videotime'=>0));
    }
}
/** 视频时间清零 */
...
/** 集体学习 */
	if(isset($_SESSION['collectiveLearn']) && $_SESSION['collectiveLearn'])
	{
		...
		{
			$lesson->update_effective_access($key);
		}
	}
	/** 集体学习 */
...
if (count($page->answers) > 0) {
    ...
    /** 跳转到下一个视频 */
    if($autocontinue==1){
        $result = $page->record_attempt_autocontinue($context,$autocontinue,$currenturl1);
    }
    else{
        $result = $page->record_attempt($context);
    }

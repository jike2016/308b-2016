...
class lesson extends lesson_base {
        ...
        $DB->insert_record('lesson_timer', $startlesson);
         ...
         if(isset($_SESSION['collectiveLearn']) && $_SESSION['collectiveLearn']) {
            foreach ($_SESSION['collectiveIDs'] as $key) {
                $temp = $startlesson->userid;
                $startlesson->userid = $key;//stud4
                $DB->insert_record('lesson_timer', $startlesson);
                $timer->userid = $temp;
            }
        }
    ...
    public function update_timer($restart=false, $continue=false, $endreached =false) {
         ...
         if(isset($_SESSION['collectiveLearn']) && $_SESSION['collectiveLearn'])
        {
            foreach($_SESSION['collectiveIDs'] as $key)
            {
                $params1 = array("lessonid" => $this->properties->id, "userid" => $key);
                $timer1 = $DB->get_records('lesson_timer', $params1, 'starttime DESC', '*', 0, 1);
                $timer1 = current($timer1); // This will get the latest start time record.
                $timer1->starttime = $timer->starttime;
                $timer1->lessontime = $timer->lessontime;
                $timer1->completed =  $timer->completed;
                $DB->update_record('lesson_timer', $timer1);
            }
        }
        /** 集体学习 */
...
abstract class lesson_page extends lesson_base {
    ...
    final public function record_attempt_autocontinue($context,$autocontinue,$currenturl1) {
        global $DB, $USER, $OUTPUT, $PAGE;
         ...
         * against what ever custom criteria they have defined
         */
        $result = $this->check_answer_autocontinue($autocontinue,$currenturl1);

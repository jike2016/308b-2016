...
class lesson_page_type_branchtable extends lesson_page {
    ...
    public function display($renderer, $attempt) {
		...
		/** START 叶靖华 视频播放页面用户区分输出 20160227*/
        if (($PAGE->user_allowed_editing())){
            $buttonshtml = str_replace('<form method="post"', '<form id="autocontinue" method="post" ', $buttonshtml);
            $output .= $renderer->heading(format_string($this->properties->title), 3);
            $output .= $buttonshtml;
            $output .= $renderer->box($this->get_contents(), 'contents');
        }
        else {
            //为form添加id，给js获取，播放视频完毕自动提交
            $button = str_replace('<div class="singlebutton"><form method="post"',
                '<div class="btn singlebutton"><form id="autocontinue" method="post" ', $buttons[0]);
            //$button = str_replace('<input type="submit"','<input type="submit" class="btn-danger" ', $button);
            $output .= html_writer::start_tag('div', array('class' => 'videotitle-son'));
            $output .= html_writer::start_tag('p');
            $output .= format_string($this->properties->title);
            $output .= html_writer::end_tag('p');
            $output .= $button;
            $output .= html_writer::end_tag('div');
            $output .= html_writer::start_tag('div', array('class' => 'main'));
            $output .= html_writer::end_tag('div');
            $output .= html_writer::start_tag('div', array('class' => 'videobox'));
            $output .= $renderer->box($this->get_contents(), 'contents');
            $output .= html_writer::end_tag('div');

            $output .= html_writer::start_tag('div', array('class' => 'commentbox'));
            $output .= html_writer::start_tag('div', array('class' => 'commentboxtitle'));
            $output .= html_writer::start_tag('div');
            $output .= $renderer->heading("评论", 3);
            $output .= html_writer::end_tag('div');
            $output .= html_writer::end_tag('div');
            ...
            $output .= '<div class="comment container">
				<div class="comment-l">
					<div class="Learnerimg-box">
						<img src="' . $CFG->wwwroot . '/theme/more/pix/lessonvideo/learnner.jpg" alt="依米暖暖">
					</div>
				</div>
				<div class="comment-r">
					<p class="name">红桃A</p>
    ...
    public function check_answer() {
        ...
        $DB->insert_record("lesson_branch", $branch);
		 ...
		 /** 集体学习 */
         if(isset($_SESSION['collectiveLearn']) && $_SESSION['collectiveLearn'])
        {
            foreach($_SESSION['collectiveIDs'] as $key)
            {
                $temp = $branch->userid;
                $branch->userid = $key;
                $DB->insert_record("lesson_branch", $branch);
    ...
    public function check_answer_autocontinue($aotucontinue,$currenturl1) {
        global $USER, $DB, $PAGE, $CFG;
        ...
        /** 集体学习 */
        //redirect(new moodle_url('/mod/lesson2/view.php', array('id' => $PAGE->cm->id, 'pageid' => $newpageid)));
        /** 跳转到下一个视频 */
        //$url='../ckplayer/VideoJmp.php?url=http://localhost/moodle/mod/lesson/view.php?id=60';
        $url = new moodle_url('/mod/lesson/ckplayer/VideoJmp.php?url='.$currenturl1);

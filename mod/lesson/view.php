...
//** Start 岑霄 1.27学生视频页面 */
if(!$canmanage){
    $PAGE->set_pagelayout('lessonvideoforstud');//设置layout文件
    //默认为incourse
}
//** End */
...
if ($pageid != LESSON_EOL) {
    ...
    if (is_array($page->answers) && count($page->answers)>0) {
        ...
        /**Start 视屏页面判断 徐东威 20160307 */
        if(!$canmanage){//学生页面
            $lessoncontent = $lessonoutput->my_display_page($lesson, $page, $attempt);//显示视屏内容
        }else{//管理员页面
	...
	echo $lessonoutput->my_header($lesson, $cm, $currenttab, $extraeditbuttons, $lessonpageid, $extrapagetitle);//layout输出
    if ($attemptflag) {
    ...
    echo $lessoncontent;
    //echo $lessonoutput->progress_bar($lesson);//课程完成进度
    echo $lessonoutput->footer();

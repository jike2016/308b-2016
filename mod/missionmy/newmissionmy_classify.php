...
$PAGE->set_url('/mod/missionmy/newmissionmy_classify.php');
$PAGE->set_title('创建台账任务');
$PAGE->set_heading('创建台账任务');
$PAGE->set_pagelayout('missionmy_classify');//设置layout文件
require_login();//要求登录
global $DB;
...
//		$this->add_action_buttons(false, '添加人员');
		...
		/** Start 隐藏的任务所属单位id */
		$attributes='id="id_hidden_orgID"';
		$mform->addElement('hidden', 'hidden_orgID', '所属单位',$attributes);
		/**End */
...
if ($mform->is_cancelled()) {
	//按了取消按钮
	redirect(new moodle_url('/mod/missionmy/index_classify.php'));
	...
	$missionArray->org_id = $fromform->hidden_orgID;//获取分级管理员所在单位
...
//	}
	...
	redirect(new moodle_url('/mod/missionmy/index_classify.php', array('id' => $newid)));

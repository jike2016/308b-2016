...
class mod_openmeetings_mod_form extends moodleform_mod {
	function definition() {
		...
		$mform->addElement('text', 'name', '直播名称', array(
				'size' => '64'
		));
		...
		/** Start 增加开始时间 岑霄 20160229*/
		$mform->addElement('date_time_selector', 'timeopen', '开始时间');//注意：这里的控件名称要与数据库的字段对应才能自动填充
		/** End */
		/** Start 增加讲课人 岑霄 20160229*/
		...
		$mform->addElement('select', 'type', '直播类型', array(
					'3' => '直播'
		...
		$mform->addElement('select', 'max_user', '最大参与人数', array(
				'150' => '150',
				'100' => '100',
				'50' => '50',
				'36' => '36',
		...
		$mform->addElement('select', 'language', '语言', $language_array);
		...
		// ));
		$mform->addElement('select', 'allow_recording', '允许录像', array(
				'1' => '允许',
				'2' => '不允许'
		));

		$mform->addElement('select', 'whole_window','显示方式', array(
				'2' => '打开一个新页面',
				'0' => '嵌入网页',
				'1' => '占据整个窗口'
		...
		$this->standard_intro_elements('描述');
		...
		$dgrp = array();
		$dgrp[] = & $mform->createElement('static', 'description', '', '只有选择了"允许录像"功能的直播视频才会出现在这里');
		$dgrp[] = & $mform->createElement('select', 'room_recording_id', get_string('recordings_show', 'openmeetings'), $recordings);
		$dgrp[] = & $mform->createElement('submit', 'avi', '下载格式.avi');
		$dgrp[] = & $mform->createElement('submit', 'flv', '下载格式.flv');
		$mform->addGroup($dgrp, 'dgrp', '录好的视频文件', array(' '), false);

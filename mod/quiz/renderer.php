...
class mod_quiz_renderer extends plugin_renderer_base {
    ...
    public function finish_review_link(quiz_attempt $attemptobj) {
        ...
        if ($attemptobj->get_access_manager(time())->attempt_must_be_in_popup()) {
            ...
            /** Start 岑霄1.27 结束回顾 */
            return '<h3><div style="text-align: center;">'.html_writer::link($url, get_string('finishreview', 'quiz')).'</div></h3>';
            /** End */
            //return '<h3><a href="http://localhost/moodle/mod/quiz/view.php?id=72">结束回顾</a></h3>';
        }
    }
    ...
    public function attempt_form($attemptobj, $page, $slots, $id, $nextpage) {
        ...
        /** Start 20160113 生成统一考试试题  韦鲲鹏 **/
        if(isset($_SESSION['unified_quiz'])&&$_SESSION['unified_quiz']) {
            foreach ($slots as $slot) {
                $attemptobj->render_question($slot, false, $this,
                    $attemptobj->attempt_url($slot, $page), $this);
            }
            $_SESSION['unified_quiz'] = false;
            /** Start 20160113 随机题改成固定题  岑霄 **/
            global $DB;
            global $USER;
            $random_questions = $DB->get_records('quiz_slots', array('quizid'=>$attemptobj->get_quizobj()->get_quiz()->id));
            $user_attempt = $DB->get_record('quiz_attempts', array('quiz'=>$attemptobj->get_quizobj()->get_quiz()->id,'userid'=>$USER->id));
            $fixed_questions = $DB->get_records('question_attempts', array('questionusageid'=>$user_attempt->uniqueid));
            $questions=array();
            foreach($fixed_questions as $fixed_question){
                $questions[]=$fixed_question->questionid;
            }
            $i=0;
            foreach($random_questions as $random_question){
                $DB->update_record('quiz_slots', array('id'=>$random_question->id,'questionid'=>$questions[$i]));
                $i++;
            }
            /** End **/
    ...
    public function summary_page($attemptobj, $displayoptions) {
        $output = '';
        $output .= $this->header();
        $output .= '<div style="text-align: center;">'.$this->heading(format_string($attemptobj->get_quiz_name())).'</div>';
        //$output .= '<h2>'.$this->box(format_string($attemptobj->get_quiz_name()), 'quizinfo').'</h2>';
    ...
    public function view_page($course, $quiz, $cm, $context, $viewobj) {
        $output = '';
        /** Start 20160114 生成统一考试试题按钮  岑霄 **/
		$output .= $this->my_unified_quiz($course, $quiz, $cm, $context, $viewobj);
        /** End **/
    ...
    /** Start 20160114 生成统一考试试题按钮  岑霄 **/
	public function my_unified_quiz($course, $quiz, $cm, $context, $viewobj){
		$output = '';
		//添加统一考试按钮
		if (has_capability('mod/quiz:manage', $context)) {//判断权限
			//加个判断：考试前2分钟，且quize_attempt表里没有这个考试的记录，且考试有开始时间的。显示生成试卷按钮
            global $DB;
            $attempt_exist=$DB->record_exists('quiz_attempts', array('quiz'=>$quiz->id));
            $timenow= time();
            if($quiz->timeopen!=0&&$timenow+120>$quiz->timeopen&&$timenow<$quiz->timeopen&&!$attempt_exist&&$quiz->typeofquiz==1){
            //if(time()+120>$quiz->timeopen){
                $url = new moodle_url('/mod/quiz/startattempt.php', array('cmid'=>$cm->id,'unified_quiz'=>true));//传一个参数过去生成试卷
                $button = new single_button($url, '生成统一考试试题');
    ...
    public function view_information($quiz, $cm, $context, $messages) {
        ...
        /** Start 岑霄1.27 标题居中 */
        $output .= '<h2>'.$this->box((format_string($quiz->name)), 'quizinfo').'</h2>';

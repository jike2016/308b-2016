...
class quiz_grading_report extends quiz_default_report {
    ...
    public function display($quiz, $cm, $course) {
        ...
        // Get the list of questions in this quiz.//获取quiz的试题列表
       ...
       /* if ($data = data_submitted() && confirm_sesskey() && $this->validate_submitted_marks()) {
        ...
        /** Start 获取提交过来的成绩，做多人评分的判断 韦鲲鹏 岑霄 $data */
        // Process any submitted data.
         if ($data = data_submitted() && confirm_sesskey() && $this->validate_submitted_marks()) {
             //代码添加在这里
             if($qubaids_my = $this->my_avggrade_handle())//根据多人评分的处理结果，重新生成需要操作总分的id，按正常流程往下提交，评总分和修改总分
             {
                 $this->process_submitted_data_my($qubaids_my);
                // $this->process_submitted_data();//正常提交页面
             }
             redirect($this->grade_question_url($slot, $questionid, $grade, $page + 1));//重新载入页面
         }
         /** End **/
    ...
    /** Start20160114 处理平均分,多位老师打完分并计算出平均分并改了post的分数再返回新的$qubaids_my 岑霄  **/
    public function my_avggrade_handle(){
        $qubaids = optional_param('qubaids', null, PARAM_SEQUENCE);
        $slot = optional_param('slot', null, PARAM_INT);
        $qubaids_my = null;//重新生成需要提交的id，返回
        if (!$qubaids) {
            return false;
        }
        ...
        foreach ($qubaids as $qubaid) {
            $count_qubaid = $DB->count_records('question_avggrade_my',array('uniqueid'=>$qubaid,'slot'=>$slot));//当前已评分人数
            $grade=$_POST['q'.$qubaid.':'.$slot.'_-mark'];//获取分数
            //查数据库看当前老师有没有打分，没打分的插入，打过分的修改成绩，最后判断够多人的话计算平均分返回true，不够的话返回false
            if($avgid =$DB->record_exists('question_avggrade_my', array('uniqueid'=>$qubaid,'slot'=>$slot,'userid'=>$USER->id))){
                if($grade==null){//如果传入的成绩是空值，则删掉average表里的记录
                    $DB->delete_records("question_avggrade_my",array('uniqueid'=>$qubaid,'slot'=>$slot,'userid'=>$USER->id));
                    $count_qubaid--;
                    $qubaids_my[]=$qubaid;//也要处理，清空总分
                }
                else{//更新成绩
                    $avgid=$DB->get_record('question_avggrade_my', array('uniqueid'=>$qubaid,'slot'=>$slot,'userid'=>$USER->id), '*', MUST_EXIST);
                    $DB->update_record('question_avggrade_my', array('id'=>$avgid->id,'grade'=>$grade));
                }
            }
            else{//没有记录
                if($grade==null){//如果传入的成绩是空值，不做操作
                }
                elseif($count_qubaid>=$this->quiz->numofsubjuser){//已够或多出规定的评分人数，不做操作
                }
                else{//插入数据
                    $DB->insert_record('question_avggrade_my',array('uniqueid'=>$qubaid,'slot'=>$slot,'userid'=>$USER->id,'grade'=>$grade),true);
                    $count_qubaid++;
                }
            }
            //判断是否满足评分人数，重新生成$qubaids并返回，
            if($this->quiz->numofsubjuser == $count_qubaid){
                $qubaids_my[]=$qubaid;
                //计算平均分，修改post值
                $avggrade = $this->my_calculate_avggrade($qubaid,$slot,$count_qubaid);
                $_POST['q'.$qubaid.':'.$slot.'_-mark']=$avggrade;
            }
        }
        //return $qubaids_my;//返回需要处理的id
        return $qubaids_my;
    }
    /** End **/
    /** Start 计算平均分 岑霄 */
    public function my_calculate_avggrade($qubaid,$slot,$num){
        global $DB;
        $grades = $DB->get_records('question_avggrade_my', array('uniqueid'=>$qubaid,'slot'=>$slot));
        $sumgrade=0;
        foreach ($grades as $grade) {
            $sumgrade=$sumgrade+$grade->grade;
        }
        $avggrade=round($sumgrade/$num,2);//保留2位小数
        return $avggrade;
    }
    /** End */
    ...
    protected function display_grading_interface($slot, $questionid, $grade,
        ...
        /** Start 在这里修改展示的题号，$qubaids=uniqueid  岑霄*/
        //减去userid已评分的
        if($grade=='needsgrading'){
            list($qubaids, $count) = $this->my_subtract_needgrading($qubaids, $count,$slot);
        }

        //$qubaids=array('103','102');
        /** End  */
    ...
    /** Start 减去avg表已评分的  */
    public function my_subtract_needgrading($qubaids,$count,$slot){
        global $DB;
        global  $USER;
        $my_qubaids=null;
        $my_count=null;
        foreach($qubaids as $qubaid){
            if(!$DB->record_exists('question_avggrade_my', array('uniqueid'=>$qubaid,'slot'=>$slot,'userid'=>$USER->id))){
                $my_qubaids[]=$qubaid;
                $my_count++;
            }
        }
    ...
    protected function process_submitted_data_my($qubaids) {
        global $DB;
        //$qubaids = optional_param('qubaids', null, PARAM_SEQUENCE);
        ...
        //$qubaids = clean_param_array(explode(',', $qubaids), PARAM_INT);//不需要将字符串转化成数组了
        $attempts = $this->load_attempts_by_usage_ids($qubaids);
        $events = array();

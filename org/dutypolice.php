...
/** 查看警长直属干警*/
function showchildrenpolice()
{
    global $DB;
    if(isset($_POST['id']) &&!empty($_POST['id']))
    {
        $result = $DB->get_records_sql(sprintf('SELECT a.id, a.firstname, a.lastname, b.`data` FROM mdl_user AS a JOIN mdl_user_info_data AS b ON a.id = b.userid WHERE b.fieldid = 3 AND a.id IN (SELECT c.user_id FROM mdl_police_connection AS c WHERE c.police_id = %s)', $_POST['id']));
        if(count($result))
        {
//			格式化输出数据
...
function submitdelete()
{
    ...
    if(isset($_POST['id']) &&!empty($_POST['id']))
    {
        ...
        {
            $sv = $DB->execute(sprintf('DELETE FROM mdl_police_connection WHERE police_id = %s AND user_id = %s', $_POST['id'], $value));
        }
//			格式化输出数据
        if ($sv)
        {

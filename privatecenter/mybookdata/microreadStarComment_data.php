...
//		$(this).parent('.footer').parent('.footer-box').parent().load("mybookdata/comment_data.php?page="+page);
		$(this).parent('.footer').parent('.footer-box').parent().load("mybookdata/microreadStarComment_data.php?page="+page+'&start_time=<?php echo $start_time;?>&end_time=<?php echo $end_time;?>');
	});
...
echo_comments($page,$start_time,$end_time);//输出星评列表
...
function echo_comments($page,$start_time,$end_time){
	...
	$comments= $DB->get_records_sql("
		SELECT d.scoretime,d.docid,null AS ebookid,d.score FROM mdl_doc_score_my d WHERE d.userid = $userID AND scoretime BETWEEN $start_time AND $end_time
		UNION ALL
		SELECT e.scoretime,null AS docid,e.ebookid,e.score FROM mdl_ebook_score_my e WHERE e.userid = $userID AND scoretime BETWEEN $start_time AND $end_time

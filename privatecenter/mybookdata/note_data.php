...
//		$(this).parent('.footer').parent('.footer-box').parent().load("mybookdata/note_data.php?page="+page);
		$(this).parent('.footer').parent('.footer-box').parent().load("mybookdata/note_data.php?page="+page+'&start_time=<?php echo $start_time;?>&end_time=<?php echo $end_time;?>');
	});
...
<?php
echo_notes($page,$start_time,$end_time);//输出笔记列表

/**STATR  输出笔记列表*/
function echo_notes($page,$start_time,$end_time){
	$numofpage=15;
	...
	$userID = $USER->id;
	$notes = $DB->get_records_sql("select * from mdl_note_my m where m.userid = $userID and time between $start_time and $end_time order by time desc limit $offset,$numofpage");//分页查询，获取课程笔记中的10条记录
	$notescount = $DB->get_record_sql("select count(*) as record_count from mdl_note_my m where m.userid = $userID and time between $start_time and $end_time ");
	...
	foreach($notes as $note){
		if(isset($note->courseid)){
			...
			$course = $DB->get_record_sql('select m.id,m.fullname from mdl_course m where m.id = '.$courseID);//获取课程名称

...
function echo_quiz($categoryid,$page){
	...
	if($categoryid==0){
		...
		select
		d.id,c.fullname,d.name,d.timeclose,f.id as moduleid
		...
		where a.userid='.$USER->id.' and d.timeopen!=0 and d.timeclose!=0 and d.timeclose > '.time().' and d.attempts=1 and d.typeofquiz=2 and d.id not in
			(
				select quiz from mdl_quiz_attempts where userid='.$USER->id.' GROUP BY quiz
			)
		GROUP BY d.id
		limit '.$offset.',10
		');
	...
	foreach($quizs as $quiz){
		...
		echo '<tr>
					<td>'.$n.'</td>
					<td>'.userdate($quiz->timeclose,'%Y-%m-%d %H:%M').'</td>
					<td>'.$quiz->fullname.'</td>
					<td>'.$quiz->name.'</td>
					<td></td>
					<td><a href="../mod/quiz/view.php?id='.$quiz->moduleid.'" target="_blank"><button class="btn btn-primary checkexam">开始考试</button></a></td>
				</tr>
		';
		$n++;
	}
	...
	echo_end($page,$quizscount);//输出上下页按钮等
}
/**END  输出考试列表*/

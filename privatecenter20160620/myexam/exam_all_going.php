...
echo_quiz($categoryid,$page);//输出考试列表
...
function echo_quiz($categoryid,$page){
	...
	if($categoryid==0){
		$quizs = $DB->get_records_sql('
			...
			where a.userid='.$USER->id.' and d.timeopen < '.time().' and d.timeclose > '.time().' and d.attempts=1 and d.typeofquiz=1
			GROUP BY d.id
			limit '.$offset.',10
		');
	}
	...
	foreach($quizs as $quiz){
		...
		echo '<tr>
					...
					<td>'.$quiz->name.'</td>
					<td></td>
					<td><a href="../mod/quiz/view.php?id='.$quiz->moduleid.'" target="_blank"><button class="btn btn-primary checkexam">进入考试</button></a></td>
				</tr>
		';
		$n++;
	}

...
function echo_quiz($categoryid,$page){
	...
	if($categoryid==0){
		...
		select
		d.id,c.fullname,d.name,d.timeopen
		...
		where a.userid='.$USER->id.' and d.timeopen!=0 and d.timeclose!=0 and d.timeclose > '.time().' and d.attempts=1 and d.typeofquiz=1 and d.id not in
			(

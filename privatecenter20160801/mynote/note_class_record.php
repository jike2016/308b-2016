...
<script type="text/javascript">
	...
	$('.pre-btn').click(function() {  //上一页
		...
		//alert(page);
		page--;
		$(this).parent('.footer').parent('.footer-box').parent('.maininfo-box').load("mynote/note_class_record.php?page="+page);
	});
		...
		$.ajax({
			...
			success: function(msg){
				if(msg == 1){
					// alert('删除成功');
					var page=parseInt($('#pageid').text());
					$('.delete_btn').parent().parent().parent().parent('.table-hover').parent('.classinfo-box').parent('.maininfo-box').load("mynote/note_class_record.php?page="+page);
				}else{
					 alert('删除失败');
					 $('.lockpage').hide();
				}
...
echo_notes($categoryid,$page);//输出笔记列表
...
function echo_notes($categoryid,$page){
	...
	$userID = $USER->id;
	$notes = $DB->get_records_sql("select * from mdl_note_my m where m.userid = $userID and m.notetype = 1 order by time desc limit $offset,10");//分页查询，获取课程笔记中的10条记录
	$notescount = $DB->get_record_sql("select count(*) as record_count from mdl_note_my m where m.userid = $userID and m.notetype = 1 ");
	$no = ($page-1)*10+1;//序号
	...
	echo_end($page,$notescount);//输出上下页按钮

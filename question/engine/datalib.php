...
ORDER BY
    ...
    q.id
        ", $params + $qubaids->from_where_params());
       // $aa=$params + $qubaids->from_where_params();
        $results = array();
        foreach ($rs as $row) {
            ...
            if (!array_key_exists($index, $results)) {
                $res = new stdClass();
                $res->slot = $row->slot;//位置
                $res->questionid = $row->questionid;//试题id
                $res->name = $row->name;//试题名称
                $res->inprogress = 0;
                $res->needsgrading = 0;
                $res->autograded = 0;
            ...
            /** Start 先清零，然后重新计算 岑霄 */
            $results[$index]->needsgrading = 0;
            $results[$index]->manuallygraded = 0;
            /** End */
        }
        $rs->close();
        /** Start 重新计算 岑霄 */
        //更新待评分数量
        $results_my = $this->my_needsgrading_handler($results,$slots,$qubaids,$slottest,$params);
        //更新已评分数量
        $results_my = $this->my_manuallygraded_handler($results,$slots,$qubaids,$slottest,$params);
        /** End */
        //return $results;
        return $results_my;
    }
    /** Start 对查询结果更新待评分数量 岑霄 */
    public function my_needsgrading_handler($results_my,$slots,$qubaids,$slottest,$params){
        $rs = $this->my_needsgrading_reload($slots,$qubaids,$slottest,$params);
        foreach ($rs as $row) {
            $index = $row->slot . ',' . $row->questionid;
            $results_my[$index]->needsgrading = $row->numattempts;
        }
        $rs->close();
        return $results_my;
    }
    /** End */
    /** Start 重新读取needsgrading的数目  岑霄*/
    public function my_needsgrading_reload($slots,$qubaids,$slottest,$params){
        global $USER;
        list($slottest2, $params2) = $this->db->get_in_or_equal($slots, SQL_PARAMS_NAMED, 'slot2');//再取一次。
        $quizid=$qubaids->from_where_params();
        $quizid=$quizid['mangrquizid'];
        //$rs = $this->db->get_recordset_sql("
        $rs = $this->db->get_recordset_sql("
SELECT
    qa.slot,
    ...
    qa.slot $slottest AND
		...
		AND qa.id NOT IN (
				SELECT
					...
					JOIN mdl_question q ON q.id = qa.questionid
					JOIN mdl_question_avggrade_my qam ON qam.uniqueid=quiza.uniqueid AND qam.slot= qa.slot AND qam.userid=$USER->id

					WHERE
					  quiza.quiz = $quizid AND
					  quiza.preview = 0 AND
					  quiza.state = 'finished' AND
    qa.slot $slottest2 AND
		qas.state='needsgrading')
GROUP BY
	qa.slot,
    qa.questionid
ORDER BY
    ...
    q.name,
    q.id;
         ", $params + $qubaids->from_where_params()+$params2);
        return $rs;
    }
    /** End */
    /** Start 对查询结果更新已评分数量 岑霄 */
    public function my_manuallygraded_handler($results_my,$slots,$qubaids,$slottest,$params){
        $rs = $this->my_manuallygraded_reload($slots,$qubaids,$slottest,$params);
        foreach ($rs as $row) {
            $index = $row->slot . ',' . $row->questionid;
            if(isset( $results_my[$index])){
                $results_my[$index]->manuallygraded = $row->numattempts;
            }
        }
        $rs->close();
        return $results_my;
    ...
    /** End */
    /** Start 重新读取manuallygraded的数目 岑霄 */
    public function my_manuallygraded_reload($slots,$qubaids,$slottest,$params){
        global $USER;
        $quizid=$qubaids->from_where_params();
        $quizid=$quizid['mangrquizid'];
        $rs = $this->db->get_recordset_sql("
SELECT
		...
		COUNT(1) AS numattempts
FROM mdl_quiz_attempts quiza
JOIN mdl_question_avggrade_my qam
		ON quiza.uniqueid=qam.uniqueid
JOIN mdl_question_attempts qa
		ON qa.questionusageid=qam.uniqueid
		AND qa.slot=qam.slot

where quiza.quiz=$quizid
	AND qam.userid=$USER->id
...
$sqlorderby
        ", $params);
        /** Start 重新找出userid已评分的 岑霄 */
        if($summarystate=='manuallygraded'){
            //list($qubaids, $count) = $this->my_handle_manuallygraded($qubaids, $count,$slot);
            $qubaids = $this->my_manuallygraded_question_reload($quizid,$slot,$questionid);
        }
        /** End */
    ...
    /** Start 重新查询已评分题目 岑霄 */
    public function my_manuallygraded_question_reload($quizid,$slot,$questionid){
        global  $USER;
        $my_qubaids=null;
        //$my_count=null;
        $my_qubaids = $this->db->get_records_sql_menu("
SELECT
    qam.uniqueid
FROM mdl_question_avggrade_my qam
JOIN mdl_quiz_attempts quiza ON quiza.uniqueid= qam.uniqueid
JOIN mdl_question_attempts qa ON qa.questionusageid= qam.uniqueid AND qa.slot= qam.slot
...
");
        //$my_qubaids = array_keys($my_qubaids);
        //$my_count=count($my_qubaids);
       // return array($my_qubaids,$my_count);
        return $my_qubaids;
    }
    /**

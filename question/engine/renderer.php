...
class core_question_renderer extends plugin_renderer_base {
    ...
    public function question(question_attempt $qa, qbehaviour_renderer $behaviouroutput,
        ...
        $output .= html_writer::nonempty_tag('div',
                ...
                array('class' => 'outcome'));
        $output .= html_writer::nonempty_tag('div',//输出分数
    ...
    public function standard_mark_out_of_max(question_attempt $qa, question_display_options $options) {
        ...
        /** Start 重新查询分数 岑霄 */
        $a->mark = $this->my_reload_grade_avg($qa);
        /** End */
        return get_string('markoutofmax', 'question', $a);
    }
    /** Start 重新去avg表查询分数 岑霄*/
    public function my_reload_grade_avg($qa){
        global $DB;
        global $USER;
        $rs = $DB->get_record_sql("SELECT grade FROM mdl_question_avggrade_my WHERE uniqueid={$qa->get_usage_id()} AND slot={$qa->get_slot()} AND userid=$USER->id ");
        return number_format($rs->grade, 2, '.', '');
    }
    /** End */

...
$returnurl = new moodle_url('/register/admin_check_classify.php');
require_login();//要求登录
/**start  权限判断 只允许分级管理员 */
global $USER;
...
$PAGE->set_url('/register/admin_check_classify.php');
$PAGE->set_title('注册审核');
...
function my_get_check_count($check_type,$eachpage,$orgIDStr)    //获取审核数目的页码
{
    global $DB;
    if ($check_type==-1){
        $check_count = $DB->get_records_sql('SELECT id as mycount FROM mdl_register_temp_user where org_id in ('.$orgIDStr.') order by timecreated desc ;');
    }else {
        $sql = 'SELECT id as mycount FROM mdl_register_temp_user WHERE admin_check='.$check_type.' and org_id in ('.$orgIDStr.') order by timecreated desc;';
        $check_count = $DB->get_records_sql('SELECT id as mycount FROM mdl_register_temp_user WHERE admin_check='.$check_type.' and org_id in ('.$orgIDStr.') order by timecreated desc;');
    }
...
function my_get_check($check_type, $current_page, $eachpage,$orgIDStr) //获取审核条目
{
    ...
    if ($check_type==-1) {  //从数据库获取条目  按时间戳降序排序
        $temp_users = $DB->get_records_sql('select * FROM mdl_register_temp_user where org_id in ('.$orgIDStr.') order by timecreated DESC LIMIT '.$my_page.','.$eachpage.';');
    }else{
        $temp_users = $DB->get_records_sql('select * FROM mdl_register_temp_user WHERE admin_check=' . $check_type . ' and org_id in ('.$orgIDStr.') order by timecreated desc LIMIT '.$my_page.','.$eachpage.';');
    }
    $current_page++;
...
function my_get_check_current_count($count_page, $check_type)   //输出页码
{
    ...
    {
        echo'<a href="../register/admin_check_classify.php?check_type='.$check_type.'&page='.$num.'">'.$num.'</a>  ';
    }
}
...
$currenturl = new moodle_url('/register/admin_check_classify.php');
echo $OUTPUT->header();
echo '<h2>注册审核</h2>';
$mform = new admin_check_form($currenturl);
global $DB;
$count_page=my_get_check_count($check_type,$eachpage,$orgIDStr);
$table=my_get_check($check_type,$current_page-1,$eachpage,$orgIDStr);

if ($fromform = $mform->get_data()) {
    redirect(new moodle_url('/register/admin_check_classify.php?check_type='.$fromform->check_type));
}else{
    $mform->display();
    echo $htmltable = html_writer::table($table);
    if ($count_page>1) {
        echo '<div>
                  <a href="../register/admin_check_classify.php?check_type=' . $check_type . '&page=1">首页</a>
                   <a href="../register/admin_check_classify.php?check_type=' . $check_type . '&page=' . ($current_page <= 1 ? 1 : $current_page - 1) . '">上一页</a>  ';
        my_get_check_current_count($count_page,$check_type);

        echo '<a href="../register/admin_check_classify.php?check_type=' . $check_type . '&page=' . ($current_page < $count_page ? ($current_page + 1) : $count_page) . '">下一页</a>
                <a href="../register/admin_check_classify.php?check_type=' . $check_type . '&page=' . $count_page . '">尾页</a>
                                 </div>';
    }
}

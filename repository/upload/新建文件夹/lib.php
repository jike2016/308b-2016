        ...
        if($fileType == 'mp4' || $fileType =='flv'){
            if (repository::draftfile_exists($record->itemid, $record->filepath, $record->filename)) {
                ...
                if ($overwriteexisting) {
					...
					file_put_contents('2016text.txt',$url);
...
/*
    ...
    public function my_uploadFile($ftpHost,$fileURL,$fileType,$filename){
		...
		$ftp_server = "10.148.216.167";//服务器IP,根据实际IP设置
        $ftp_user = "gcmooc1";//用户名
        $ftp_pass = "gcmooc";//密码
        $conn_id = ftp_connect($ftp_server,21) or die("Couldn't connect to $ftp_server");//建立连接

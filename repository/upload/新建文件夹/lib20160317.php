...
class repository_upload extends repository {
    ...
    public function process_upload($saveas_filename, $maxbytes, $types = '*', $savepath = '/', $itemid = 0,
        ...
        if (repository::draftfile_exists($record->itemid, $record->filepath, $record->filename)) {
            ...
            if ($overwriteexisting) {
                ...
                $url = $this->my_uploadFile($ftpHost,$fileURL,$fileType);//文件上传
                // file_put_contents('2016.txt',$fileURL,FILE_APPEND);
     ...
     * @param  $fileType 文件类型
     */
    public function my_uploadFile($ftpHost,$fileURL,$fileType){
...
//            echo "Connected to $ftp_server, for user $ftp_user --连接信息 <br/>";
        ...
        $destination_file="$newFileName.$fileType"; //目标地址，指明传递到服务器上后的名字（相当于重命名）

...
global $USER;
$org = new org();
$admin_nodeID = $org->get_nodeid_with_userid($USER->id);//获取分级管理员所在的单位id
$role = new my_role_conf();
$remove_role = $role->get_unit_role().','.$role->get_gradingadmin_role().','.$role->get_courseadmin_role();//获取要移除的账号角色
$tree = $org->show_node_tree_user_no_office_no_grading($admin_nodeID,$remove_role);
...
//		添加人员
		function adduserSelect(userid, username)
		{
			...
			$('#id_userSelect option').each(function () {
				var $option = $(this);
			...
			$('#id_userSelect').append('<option value='+userid+'>'+username+'</option>');
		}
		...
		$(document).ready(function(){
			...
			$('#add').on('click', function(){      //选择人员的‘添加’按钮动作
				//select控件添加内容
				if(treenode_userid == 0 && treenode_orgid != 0)//选的是单位，就要根据该单位去获取其下的人员
				{
					$.ajax({
						url: "../../org_classify/orgCRUD.php",
						dataType:"json",
						data: { treeNodeid: treenode_orgid, remove_role: remove_role, type: 'click_unit_user' },
...
//								$('#id_userSelect').append('<option value='+userid+'>'+username+'</option>');
				...
				else if(treenode_orgid == 0 && treenode_userid != 0)//选的是人员
				{
			...
			$('#id_submitbutton').click(function(){   //弹出窗口
				var hidden_orgID = document.getElementById("id_hidden_orgID");//分级管理员所在的单位
				hidden_orgID.value = <?php echo $admin_nodeID; ?>;

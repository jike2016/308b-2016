...
class theme_more_core_course_renderer extends core_course_renderer {
   /**
    ...
    public function course_category($category) {
		...
		//这里是获取课程列表的地方，如果是index.php则只显示分类，如果带category参数就会显示课程摘要等信息。
		...
		if(count($_GET)==0)
		{
			$output .='<div id="j-extentionImgs">            
				<a href="http://localhost/moodle/theme/more/pix/guanggao/g1.png" target="_blank" class="custom">
				<img src="http://localhost/moodle/theme/more/pix/guanggao/g1.png" style="width: 225px;"></a>  
				<a href="http://localhost/moodle/theme/more/pix/guanggao/g2.jpg"" target="_blank" class="custom"><img src="http://localhost/moodle/theme/more/pix/guanggao/g2.jpg" style="width: 225px;"></a>    
				</div> 
				<div id="j-recomdCourse">  <p class="f-f0 t-recCourse">推荐课程</p>      
				 <a href="http://study.163.com/topics/iaskmedia/?utm_source=icourse163.org&utm_medium=web_courseindexbanner&utm_campaign=business" target="_blank" class="custom"> <dl> 
				 <dd><img src="http://localhost/moodle/theme/more/pix/guanggao/g3.png" width="225px" height="125px"><span style="background-color: #38a030;" class="org-label">网易云课堂</span></dd> 
				 <dt> <h4 class="t-h4 f-f0">徐小平：人人都可以创业</h4><p class="txt-org f-f0">艾问传媒</p> </dt> </dl> </a>      <a href="http://www.icourse163.org/course/xjtu-46019#/info" target="_blank" class="custom"> <dl> <dd>
				 <img src="http://localhost/moodle/theme/more/pix/guanggao/g4.jpg" width="225px" height="125px">
				 <span style="background-color: #ffffff;" class="org-label"></span></dd> <dt> <h4 class="t-h4 f-f0">中国哲学经典著作导读</h4><p class="txt-org f-f0">浙江大学</p> </dt> </dl> </a>  
				 </div> ';
		$output .= "</div>";
		}
        ob_start();
    ...
    public function my_frontpage_available_courses() {
        global $CFG;
        require_once($CFG->libdir. '/coursecatlib.php');
		$chelper = new theme_more_coursecat_helper();
        //$chelper = new coursecat_helper();
    ...
    protected function my_coursecat_coursebox(coursecat_helper $chelper, $course, $additionalclasses = '') {
		...
		//获取摘要里面的图片
		$content .='<div style="float:left; margin:5px 0px 0px 5px; height:160px; width:270px;"><img '.$this->my_coursecat_coursebox_content_pix($chelper, $course).' width="265px" height="160px"></div>';
		$content .='<div style="float:right; width:68%; margin:20px 5px;">
					<div style="width:100%; height:23px;"><p style="font-size:18px;">'.$coursenamelink.'</p></div>
					<div style="width:100%; height:23px; margin:5px 0px; font-size:10px;"><a href="#" class="agreen">教师：</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="#" class="agreen">胡可先、陶然</a></div>
					<div style="width:100%; height:60px; margin:15px 0px 0px; font-size:12px;" class="agreen">';
		//课程摘要
		$content .=$this->my_coursecat_coursebox_content($chelper, $course);	
		//echo substr($course->summary,4,3);
		$content .= '			
					</div>
    ...
    protected function my_coursecat_coursebox_content(coursecat_helper $chelper, $course) {
		//echo'11';exit;
		//$my_chelper=new my_coursecat_helper();
        global $CFG;
        if ($chelper->get_show_courses() < self::COURSECAT_SHOW_COURSES_EXPANDED) {
            return '';
        }
        ...
        if ($course->has_summary()) {
            ...
            $content .= $chelper->my_get_course_formatted_summary($course,
	...
	//获取摘要里面的图片
	protected function my_coursecat_coursebox_content_pix(coursecat_helper $chelper, $course) {
		//echo'11';exit;
		//$my_chelper=new my_coursecat_helper();
        global $CFG;
        if ($chelper->get_show_courses() < self::COURSECAT_SHOW_COURSES_EXPANDED) {
        ...
        if ($course->has_summary()) {
            ...
            $content .= $chelper->my_get_course_formatted_summary_pix($course);
            //$content .= html_writer::end_tag('div'); // .summary
        }  
        return $content;
    }
...
class theme_more_coursecat_helper extends coursecat_helper{
     ...
     * @return string
     */
    public function my_get_course_formatted_summary($course, $options = array()) {
        global $CFG;
		...
		$my_course_summary=mb_substr($course->summary,0,100,'utf-8').'...';
        $summary = file_rewrite_pluginfile_urls($my_course_summary, 'pluginfile.php', $context->id, 'course', 'summary', null);

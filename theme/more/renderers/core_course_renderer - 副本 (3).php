...
class theme_more_core_course_renderer extends core_course_renderer {
				...
				$output .= '<li class="course-one linefrist">
								<a href="view.php?id='.$course->id.'" target="_self">
									<div class="course-list-img">
										<img width="240" height="135" alt="'.$course->fullname.'" '.$this->my_get_course_formatted_summary_pix(new course_in_list($course)).'>
									</div>
									<h5><span>'.$course->fullname.'</span></h5>
									<div class="tips">
										<p class="text-ellipsis">'.mb_substr($course->summary,0,25,'utf-8').'</p>
		...
		<div class="course-content">
			<div class="course-tool-bar clearfix">
				<div class="tool-left l">
					...
					<a href="javascript:void(0)" class="sort-item ">最热</a>
				</div>
			...
			echo $this->my_course_category_display_indexpage(null,null,$sortkind);//全部
		echo '
	...
	//输出课程列表
	protected function my_course_category_display_indexpage($categoryid=null,$dep=null,$sortkind=null){
		global $DB;
		if($categoryid==null){//全部
			//是否排序
			if($sortkind==1){//最新

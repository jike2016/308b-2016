...
//		$content .= $this->output->box_end();
	...
	public function my_course_category($category){
		...
		//最新
		if($sortkind == 1){
			$output .= '<a href="#" class="sort-item active" onclick="sel5(1);">最新</a>';
		}else{
			$output .= '<a href="#" class="sort-item" onclick="sel5(1);">最新</a>';
		}
		//日热门
		if($sortkind == 2){
			$output .= '<a href="#" class="sort-item active" onclick="sel5(2);">周热门</a>';
		}else{
			$output .= '<a href="#" class="sort-item" onclick="sel5(2);">周热门</a>';
		}
		//周热门
		if($sortkind == 3){
			$output .= '<a href="#" class="sort-item active" onclick="sel5(3);">月热门</a>';
		}else{
			$output .= '<a href="#" class="sort-item" onclick="sel5(3);">月热门</a>';
		}
		//月热门
		if($sortkind == 4){
			$output .= '<a href="#" class="sort-item active" onclick="sel5(4);">总热门</a>';
		}else{
			$output .= '<a href="#" class="sort-item" onclick="sel5(4);">总热门</a>';
		}
//原代码：
//		if($sortkind==1){
//			$output .='
	...
	protected function my_course_category_display($categoryid=null,$dep=null,$sortkind=null){
		...
		if($categoryid==null){//全部
			...
			switch($sortkind){
				...
				case 3:	//按月热门排序
				case 4:	//按总热门排序
					$courses = $DB -> get_records_sql('SELECT a.id, a.fullname, a.summary FROM `mdl_course` a join `mdl_course_order_my` b on a.id = b.courseid WHERE a.id != 1 and a.visible=1 ORDER BY b.`'.$order_by_my[$sortkind].'` desc limit 16');
					break;
				default:	//默认排序
...
//			}
		...
		elseif($dep==1){
			...
			switch($sortkind){
				...
				case 3:
				case 4:
					$courses = $DB->get_records_sql('select distinct a.id,fullname,summary from mdl_course a join mdl_course_link_categories b join mdl_course_order_my c ON a.id = c.courseid where b.mdl_course_categories_id in('.$categoryids.') and b.mdl_course_id=a.id order by c.'.$order_by_my[$sortkind].' desc;');
					break;
				default:
...
//			}
			...
			switch($sortkind){
				...
				case 3:
				case 4:
					$courses = $DB->get_records_sql('select a.id,fullname,summary from mdl_course a join mdl_course_link_categories b join mdl_course_order_my c ON a.id = c.courseid where b.mdl_course_categories_id='.$categoryid.' and b.mdl_course_id=a.id order by c.'.$order_by_my[$sortkind].' desc;');
					break;
				default:
